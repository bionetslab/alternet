...
def as_source_vs_as_full(net2_for_t2, net3_tf_only, tx2gene, epsilon = 1e-6):
    ...
    set_b['target_tx_set'] = set_b.apply(
        lambda r: _net3_tx_map.get(r['source_transcript'] + '_' + r['target_gene'], ''), axis=1)
...
def categorize_target_resolution(edge_tg, net2_mean, net3_mean, net3_max, net3_dom,
    ...
    return {
        'source_transcript': reg_tx, 'source_gene': reg_gene, 'target_gene': target_gene,
...
def unpack_set_b(net3_tf_only, set_b):
    ...
    # Create join key
    set_b_unpacked['edge_tg'] = set_b_unpacked['source_transcript'] + '_' + set_b_unpacked['target_gene']
    
    set_b_key = set_b[['source_transcript', 'target_gene', 'target_category', 'dominance', 'S3_mean_sum', 'target_tx_set']].copy()
    set_b_key['edge_tg'] = set_b_key['source_transcript'] + '_' + set_b_key['target_gene']
...
def annotate_set_b(set_b, set_b_unpacked):
    ...
    _dominant_map = set_b_unpacked[
        ...
        set_b_unpacked[set_b_unpacked['target_rank_within_edge'] == 1].apply(
            lambda r: r['source_transcript'] + '_' + r['target_gene'], axis=1)
    )['target_transcript'].to_dict()

    set_b['target_tx_dominant'] = set_b.apply(
        lambda r: _dominant_map.get(r['source_transcript'] + '_' + r['target_gene'], ''), axis=1)
        ...
        def process_chunk(chunk):
            ...
            for row in chunk:
                result = compute_sf_splicing_evidence(
                    ...
                    row['source_gene'], row['n_target_tx'], 
                    sf_expr_dict, gene_to_all_transcripts,
                    usage_df_indexed, transcript_data, reliability_df_indexed, sample_cols
                )
                results.append(result)
            return results

        # 3. Split the data into 16 chunks (one for each core)
        n_cores = n_cores
        chunks = np.array_split(edge_records, n_cores)

        # 4. Execute the chunks in parallel
        # The output will be a list of lists, which we then flatten
        parallel_results = Parallel(n_jobs=n_cores)(
            delayed(process_chunk)(chunk) for chunk in chunks
        )

        # 5. Flatten the list of lists and build DataFrame
        flattened_results = [item for sublist in parallel_results for item in sublist]
